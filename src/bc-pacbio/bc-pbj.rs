...
fn pacbio_join() -> Result<(), failure::Error> {
    ...
    for line_res in barcodes_in.lines() {
        ...
        if fields.len() < 4 {
            ...
            let reads = barcode_to_reads
                .entry((fields[1].to_string(), fields[3].to_string()))
                .or_insert(Vec::new());
    ...
    for (&(ref library, ref barcode), &ref reads) in barcode_to_reads.iter() {
        ...
        let read_aligns: Vec<&bam::Record> = aligns
            .iter()
            .filter_map(|read_align| read_align.first())
            .collect();
        ...
        if read_aligns
            .iter()
            .all(|ra| ra.tid() == read_aligns[0].tid() && ra.pos() == read_aligns[0].pos())
        {
            write!(
                assign_out,
                "{}\t{}\t{}\t{}\n",
                ...
                barcode,
                reads.len(),
                format_align(&target_names, read_aligns[0])?
            )?;
            write!(
                barcode_out,
                "{}\t{}\t{}\tGood\n",
