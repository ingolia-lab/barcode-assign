...
fn main() {
    let facs_spec = LibSpec::new(FlankMatchSpec::new(b"GCTCGGAGATGTGTATAAGAGACAG",
                                                b"CTGTCTCTTATACACATCTGACGCC",
                                                4),
                                 FlankMatchSpec::new(b"CTATAGCACGACGCTCTTCCGATCT",
                                                b"GATCCTGTAGCCCTAGACTTGATAG",
                                                4),
                                 false);

    let syntf_spec = LibSpec::new(FlankMatchSpec::new(b"CTCGGAGATGTGTATAAGAGACAG",
                                                 b"CTGTCTCTTATACACATCTGACGC",
                                                 4),
                                  FlankMatchSpec::new(b"GCGCTCTGTTGATAACTCCGGATC",
                                                 b"AGATCGGAAGAGCGTCGTGCTATA",
                                                 4),
                                  true);

    let specs = vec![(&"FACS", &facs_spec),
                     (&"SynTF", &syntf_spec)];
    ...
    for recres in fqin.records() {
        ...
        let lib_matches: Vec<(String, String, LibMatchOut)> = specs
            ...
            .flat_map(|(&name, ref spec)| {
                vec![(name.to_string(), "Fwd".to_string(), spec.best_match(&sequ_fwd)),
                     (name.to_string(), "Rev".to_string(), spec.best_match(&sequ_rev))]
            }).collect();

        let good_matches: Vec<(String, String, LibMatch)> = lib_matches.iter().filter_map(|(name, strand, lib_match_out)| {
            lib_match_out.lib_match().map(|lib_match| (name.to_string(), strand.to_string(), lib_match))
        }).collect();
