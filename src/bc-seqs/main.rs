...
fn main() {
    ...
    let barcodes = matches.value_of("bclist").map_or(outbase.to_string() + "-barcodes.txt", |s| s.to_string());
    let barcode_freqs = matches.value_of("bcfreq").map_or(outbase.to_string() + "-barcode-freqs.txt", |s| s.to_string());
    ...
    let config = Config {
        ...
        out_barcodes: if matches.is_present("no-bclist") { None } else { Some(barcodes) },
        out_barcode_freqs: if matches.is_present("no-bcfreq") { None } else { Some(barcode_freqs) },
    };
