...
impl <'a> BarcodeGroups<'a> {
    ...
    pub fn new(read_barcode: &'a Fn(&bam::Record) -> Option<&[u8]>, bam_reader: &'a mut bam::Reader) -> Result<Self, failure::Error> {
        let mut bg = BarcodeGroups{ bam_reader: bam_reader, next_record: None, read_barcode: read_barcode, };
    ...
    fn barcode_group(&mut self, curr: bam::Record) -> Result<(Vec<u8>, Vec<bam::Record>), failure::Error>
    {
        let curr_bc = (self.read_barcode)(&curr).ok_or(err_msg("No barcode for current read"))?.to_vec();
        ...
        loop {
            ...
            if let Some(rec) = next {
                ...
                let same_group = (self.read_barcode)(&rec).ok_or(format_err!("No barcode for read {:?}", rec))? == curr_bc.as_slice();
